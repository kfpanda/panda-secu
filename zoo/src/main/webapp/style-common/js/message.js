...
var templateSrc = "/style-common/images/ico_load.gif"; //设置loading.gif路径
function tipsWindown(title, content, width, height, drag, time, showbg, cssName, iframeurl, iframeheight, iframescrolling) {
	//iframe height width
...
//		jQuery('#windown-content-border').html(html)
	...
	if ("ids" == contentType) {
		if (_version == 6) {
			jQuery("#windown-box").css({left:"50%", top:(parseInt((ch) / 2) + est - 120) + "px", marginTop:-((parseInt(height) + 53) / 2) + "px", marginLeft:-((parseInt(width) + 32) / 2) + "px", zIndex:"999999"});
		} else {
			jQuery("#windown-box").css({left:"50%", top:scrollTop+300+(height/2)+"px", marginTop:-((parseInt(height) + 53 - 100) / 2) + "px", marginLeft:-((parseInt(width) + 32) / 2) + "px", zIndex:"999999"});
		}
	} else {
		if (_version == 6) {
			...
			jQuery("#windown-box").css({left:"50%", top:scrollTop+150+(height/2)+"px", marginTop:-((parseInt(height) + 53) / 2) + "px", marginLeft:-((parseInt(width) + 32) / 2) + "px", zIndex:"999999"});
		}
	}
...
function showAlertDivLayer(msg) {
	...
	var sureClickFunLayerDiv = divLayerClose;
	var height = 120;
	var width = 340;
	var scrollState = "no";
	var clickFun = false;
	...
	var s = "<div id=\"testLayerDivID\">" + "<div class=\"open_prompt\">" + "<table width=\"100%\" border=\"0\" class=\"table01\">" + "<tr>" + "<td width=\"" + magLeft + "\">" + "<div class=\"img " + img_show + "\">" + "</div>" + "</td>" + "<th width=\"220\">" + "<p>" + msg + "</p>" + "</th>" + "</tr>" + "<tr>" + "<td colspan=\"2\" align=\"center\" class=\"btn\">" + "<input id=\"testLayerDivIDSureBtn\" type=\"button\" class=\"determine\" value=\"\u786e \u5b9a\"/>" + "</td>" + "</tr>" + "</table>" + "</div>" + "</div>";
	tipsWindown("\u4fe1\u606f\u63d0\u793a", "ids:testLayerDivID", width, height, "true", "", true, "id", s, height, scrollState);
...
function showConfirmDivLayer(msg) {
	...
	var cancelClickFunLayerDiv = divLayerClose;
	var height = 100;
	var width = 260;
	var scrollState = "no";
	...
	var s = "<div id=\"testConfirmLayerDivID\">" + "<div class=\"open_prompt\">" + "<table width=\"100%\" border=\"0\" class=\"table01\">" + "<tr>" + "<th width=\"" + tdWidth + "\">" + "<p>" + msg + "</p>" + "</th>" + "</tr>" + "<tr>" + "<td colspan=\"2\" align=\"center\" class=\"btn\">" + "<input id=\"testConfirmLayerDivIDSureBtn\" type=\"button\" class=\"determine\" value=\"\u786e \u5b9a\"/>" + "<input id=\"testConfirmLayerDivIDCancelBtn\" type=\"button\" class=\"determine\" value=\"\u53d6 \u6d88\"/>" + "</td>" + "</tr>" + "</table>" + "</div>" + "</div>";
	tipsWindown("\u4fe1\u606f\u63d0\u793a", "ids:testConfirmLayerDivID", width, height, "true", "", true, "id", s, height, scrollState);
...
function showWindow(title,width,height,url){
	tipsWindown(title,'page:',width,height,'true','',true,'',url,'','no');
}

function showWindowScroll(title,width,height,url){
	tipsWindown(title,'page:',width,height,'true','',true,'',url,'','yes');
}